...
impl WorldPanel {
    ...
    pub fn update(&mut self, ui: &mut egui::Ui, level_data: &mut level_data::LevelData, editor_data: &mut EditorData) {
        // Draw the level
        let (rect, response) =
        ...
        let mut sprites = vec![];
        self.update_level_background(&mut sprites, ui, level_data, editor_data, false, &rect, &response, &pointer_pos);
        if !self.editing_background {
            self.update_level_tiles(&mut sprites, ui, level_data, &rect, &response, editor_data, &pointer_pos);
            self.update_level_spawners(&mut sprites, ui, level_data, &rect, &response, editor_data, &pointer_pos);
        }
        self.update_level_background(&mut sprites, ui, level_data, editor_data, true, &rect, &response, &pointer_pos);
        if !self.editing_background {
    ...
    fn update_camera(&mut self, ui: &mut egui::Ui, rect: &egui::Rect, response: &egui::Response, editor_data: &EditorData) {
        self.camera.set_projection(rect.width(), rect.height(), Some(32.0 * 10.0), false);
        if response.dragged() && response.dragged_by(egui::PointerButton::Middle) {
            ...
            let scaled_delta = cgmath::vec2(
                ((response.drag_delta().x * response.ctx.pixels_per_point()) / rect.width()) * size.x,
                ((response.drag_delta().y * response.ctx.pixels_per_point()) / rect.height()) * size.y,
            );
    ...
    fn get_background_pos(&self, layer: &BackgroundLayer, idx: usize, scale: f32) -> cgmath::Vector2<f32> {
        let pos = cgmath::vec2(self.background_pos[idx].x, self.background_pos[idx].y);
        let cx = if layer.scroll_speed.0 < f32::EPSILON { 0.0 } else { self.camera.pos.x / layer.scroll_speed.0 };
        let cy = if layer.scroll_speed.1 < f32::EPSILON { 0.0 } else { self.camera.pos.y / layer.scroll_speed.1 };
        cgmath::vec2((layer.origin.0 + pos.x) * scale, (layer.origin.1 + pos.y) * scale) +
            cgmath::vec2(self.camera.pos.x - cx, self.camera.pos.y - cy)
    }

    fn update_level_background(&mut self, sprites: &mut Vec<Sprite>, _ui: &mut egui::Ui, level_data: &mut level_data::LevelData, editor_data: &mut EditorData, foreground: bool, main_rect: &egui::Rect, response: &egui::Response, pointer_pos: &egui::Pos2) {
        let top_left = self.camera.get_top_left_world_space();
        let cam_size = self.camera.get_proj_size_world_space();
        ...
        for idx in self.background_range.clone() {
            ...
            if let BackgroundImage::Color(color_index) = layer.image {
                ...
                sprites.push(Sprite::new_pure_color(
                    ...
                    (pal[0] as f32 / 255.0, pal[1] as f32 / 255.0, pal[2] as f32 / 255.0, 1.0)
                ));
                continue;
            }
            if let BackgroundImage::Bitmap(rect) = layer.image {
                let padded_size = cgmath::vec2(rect.w as f32 + layer.spacing.0 as f32, rect.h as f32 + layer.spacing.1 as f32) * scale;
                ...
                let cap_width = layer.scroll_speed.0 < f32::EPSILON && rect.w >= 320 && !layer.repeat_horizontally;
                let width = if cap_width { cam_size.x } else { rect.w as f32 * scale };
                let src_width = if cap_width { 320 } else { rect.w };
                for _ in 0..(if layer.repeat_up || layer.repeat_down { (cam_size.y / padded_size.y).ceil() as i32 * 2 + 1 } else { 1 }) {
                    let mut x = start_x;
                    if y - f32::EPSILON * 2.0 > new_pos.y + padded_size.y / 2.0 && layer.repeat_up && !layer.repeat_down {
                        break;
                    }
                    for _ in 0..(if layer.repeat_horizontally { (cam_size.x / padded_size.x).ceil() as i32 * 2 + 1 } else { 1 }) {
                        ...
                        if pointer_pos.x > real_pos.x &&
                            ...
                            if cap_width {
                                ...
                                let local_pos = (
                                    (((pointer_pos.x - real_pos.x) / scale) as i32).clamp(0, rect.w - 1),
                                    (((pointer_pos.y - real_pos.y) / scale) as i32).clamp(0, rect.h - 1),
                                );
                                if !editor_data.opaques[(local_pos.0 + rect.x) as usize][(local_pos.1 + rect.y) as usize] {
                                    selected_layer = Some(idx);
                                }
                            }
                        }
                        ...
                        let mut sprite = Sprite::new(
                            ...
                            (width, height),
                            (rect.x as f32, rect.y as f32, src_width as f32, rect.h as f32)
                        );
                        ...
                        sprite.tint = [tint.0, tint.1, tint.2, ((1.0 - tint.3) - (layer.transparency as f32 / (level_data::consts::CLEAR as f32 - f32::EPSILON))).clamp(0.0, 1.0)];
                        sprites.push(sprite);
                        x += padded_size.x;
                    }
                    y += padded_size.y;
    ...
    fn update_level_tiles(&mut self, sprites: &mut Vec<Sprite>, _ui: &mut egui::Ui, level_data: &mut level_data::LevelData, rect: &egui::Rect, response: &egui::Response, editor_data: &mut EditorData, pointer_pos: &egui::Pos2) {
        ...
        let tile_placing_enabled = (response.ctx.input().pointer.primary_down() || response.ctx.input().pointer.secondary_down()) && !self.grabbing_resize;
        if matches!(editor_data.tool, Tool::Brush) {
            ...
            if let Some(copy_selection) = self.copy_selection.as_mut() {
                ...
                if !response.ctx.input().pointer.primary_down() {
                    let mut viewer_selection = Cells::new((copy_selection.2 - copy_selection.0 + 1) as usize, (copy_selection.3 - copy_selection.1 + 1) as usize);
                    level_data.cells.paste(&mut viewer_selection, (copy_selection.0, copy_selection.1), (copy_selection.2, copy_selection.3), (0, 0));
        ...
        for row in start.y..=end.y {
            for column in start.x..=end.x {
                ...
                let mut drawer = |props: &TileProperties, foreground: bool| {
                    ...
                    let tint = if self.gray_other && editor_data.foreground_placing != foreground && editor_data.light_placing == None { 0.5 } else { 1.0 };
                    let mut sprite = Sprite::new(
                        pos,
                        (32.0, 32.0),
                        (props.frames[idx].0 as f32 * 32.0, props.frames[idx].1 as f32 * 32.0, 32.0, 32.0)
                    );
                    sprite.tint = [tint, tint, tint, 1.0 - (props.transparency as f32 / (level_data::consts::CLEAR as f32 - f32::EPSILON))];
                    sprites.push(sprite);
                };
        ...
        if let Tool::Brush = editor_data.tool {
            ...
            if let Some(copy_selection) = self.copy_selection {
                sprites.push(Sprite::new_pure_color(
                    (copy_selection.0 as f32 * 32.0, copy_selection.1 as f32 * 32.0, 0.0),
                    ((copy_selection.2 - copy_selection.0 + 1) as f32 * 32.0, (copy_selection.3 - copy_selection.1 + 1) as f32 * 32.0),
                    (1.0, 1.0, 0.0, 0.4)
                ));
            }
        }
    ...
    fn show_grid(&mut self, sprites: &mut Vec<Sprite>, ui: &mut egui::Ui, level_data: &mut level_data::LevelData, editor_data: &mut EditorData, rect: &egui::Rect, response: &egui::Response, pointer_pos: &egui::Pos2) {
        ...
        if self.show_original_screen_size {
            sprites.append(&mut (Sprite::new_rect(
                (self.camera.pos.x - (320.0 / 2.0), self.camera.pos.y - (240.0 / 2.0)),
                (self.camera.pos.x + (320.0 / 2.0), self.camera.pos.y + (240.0 / 2.0)),
                cam_size.y / (32.0 * 8.0),
                (0.1, 0.3, 0.8, 0.7)
            )).to_vec());
...
fn draw_spawner(sprites: &mut Vec<Sprite>, spawner: &cnmo_parse::lparse::level_data::cnms_types::Spawner, camera: &Camera, editor_data: &EditorData, active: bool) {
    ...
    match &spawner.type_data {
        WobjType::Teleport(tele) => {
            ...
            Sprite::create_string(spawner.pos.0 - (tele_string.len() as f32 * 2.0) + 16.0, spawner.pos.1 - 5.0, 4.0, &tele_string, sprites);
        },
        ...
        WobjType::TunesTrigger { size, music_id } => {
            ...
            Sprite::create_string(spawner.pos.0 - (id_string.len() as f32 * 2.0) + half_width, spawner.pos.1 - 5.0, 4.0, &id_string, sprites);
        },
        ...
        WobjType::UpgradeTrigger { trigger_type } => {
            match trigger_type {
                ...
                UpgradeTriggerType::MaxPowerRune { skin_power_override } => {
                    ...
                    Sprite::create_string(spawner.pos.0 + 24.0 - (text.len() as f32 * 2.0), spawner.pos.1 - 5.0, 4.0, &text, sprites);
                },
            }
        },
        ...
        WobjType::TtNode { node_type } => {
            match node_type {
                ...
                &TtNodeType::Waypoint(waypoint_id) => {
                    ...
                    Sprite::create_string(spawner.pos.0, spawner.pos.1 - 5.0, 4.0, format!("Waypoint ID: {waypoint_id}").as_str(), sprites);
                },
            }
        },
        ...
        WobjType::RotatingFireColunmPiece { origin_x, degrees_per_second } => {
            draw_rect(416, 2048, 32, 32);
            let dist = spawner.pos.0 - *origin_x as f32;
            let pos_x = *origin_x as f32 + dist *
                ((editor_data.time_past.as_secs_f32() * *degrees_per_second).to_radians()).cos();
            let pos_y = spawner.pos.1 + dist *
                -((editor_data.time_past.as_secs_f32() * *degrees_per_second).to_radians()).sin();
            sprites.append(&mut Sprite::new_rect(
                (pos_x, pos_y),
        ...
        WobjType::DisapearingPlatform { time_on, time_off, starts_on } => {
            let mut sprite = Sprite::new(
                (spawner.pos.0, spawner.pos.1, 0.0),
                (32.0, 32.0),
                (352.0, 2112.0, 32.0, 32.0)
            );
            let time = editor_data.time_past.as_secs_f32().rem_euclid(*time_on + *time_off);
        ...
        WobjType::BgSpeed { vertical_axis, .. } => draw_rect(480, 3392 + (32 * *vertical_axis as i32), 32, 32),
        WobjType::BgTransparency { .. } => draw_rect(480, 3392+64, 32, 32),
        WobjType::TeleportTrigger1 { link_id, .. } |
        WobjType::TeleportArea1 { link_id, .. } => {
        ...
        WobjType::Supervirus => sprites.push(Sprite::new_pure_color((spawner.pos.0, spawner.pos.1, 0.0), (48.0, 96.0), (1.0, 0.0, 0.0, 0.8))),
        ...
        WobjType::CustomizeableMoveablePlatform { bitmap_x32, target_relative, speed, .. } => {
            draw_rect(bitmap_x32.0 as i32 * 32, bitmap_x32.1 as i32 * 32, 32, 32);
            sprites.append(&mut Sprite::new_rect(
                (target_relative.0 + spawner.pos.0, target_relative.1 + spawner.pos.1),
                (target_relative.0 + spawner.pos.0 + 32.0, target_relative.1 + spawner.pos.1 + 32.0),
                3.0,
                (1.0, 1.0, 0.0, 0.8),
            ).to_vec());
            ...
            sprites.push(Sprite::new_pure_color(
                (spawner.pos.0 + target_relative.0 * lin_pos, spawner.pos.1 + target_relative.1 * lin_pos, 0.0),
                (32.0, 32.0),
                (1.0, 1.0, 0.0, 0.4)
            ));
        },
